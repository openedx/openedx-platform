...
class XBlockSerializationTestCase(SharedModuleStoreTestCase):
    ...
    def test_html_with_static_asset_openedx_core(self):
        """
        Test the openedx-core-specific serialization of an HTML block
        """
        block_id = self.course.id.make_usage_key('html', 'just_img')  # see sample_courses.py
        html_block = modulestore().get_item(block_id)
        serialized = api.serialize_xblock_to_olx(html_block)
        serialized_openedx_core = api.serialize_modulestore_block_for_openedx_core(html_block)
        self.assertXmlEqual(
            serialized_openedx_core.olx_str,
            # For openedx-core, OLX should never contain "url_name" as that ID
        ...
        self.assertEqual(serialized.static_files, serialized_openedx_core.static_files)
