...
def _configure_index(index_name):
    ...
    Changing them on a populated index will "re-index all documents in the index",
    which can take some time, although not as much time as really re-indexing everything.
...
def reset_incremental_index_state(status_cb: Callable[[str], None] | None = None) -> None:
    # Ensure that the next index will start from scratch, because none are indexed now.
    status_cb("Resetting incremental indexing state...")
...
def reset_index(status_cb: Callable[[str], None] | None = None) -> None:
    ...
    if _index_exists(STUDIO_INDEX_NAME):
        status_cb("Deleting existing index...")
        _wait_for_meili_task(client.delete_index(STUDIO_INDEX_NAME))
    else:
        status_cb("No existing index...")

    reset_incremental_index_state(status_cb)

    status_cb("Creating new index...")
    _wait_for_meili_task(
        client.create_index(STUDIO_INDEX_NAME, {'primaryKey': 'id'})
    )

    status_cb("Configuring the new index...")
    _configure_index(STUDIO_INDEX_NAME)
...
def init_index(status_cb: Callable[[str], None] | None = None, warn_cb: Callable[[str], None] | None = None) -> None:
    ...
    if _index_exists(index_name):
        if not _is_index_configured(index_name):
            status_cb("Configuring index (this may take several minutes)...")
            # TODO: For some configuration changes, this can take several minutes if there is a lot of content.
            #       Figure out when is safe to run this (eg. it might be too long for migrations?).
            _configure_index(index_name)
        if _index_is_empty(index_name):
            warn_cb(
...
# TODO: Figure something out to avoid this being run multiple times simultaneously.
def rebuild_index(status_cb: Callable[[str], None] | None = None, warn_cb: Callable[[str], None] | None = None) -> None:  # lint-amnesty, pylint: disable=too-many-statements
    """
    ...
    # Ensure the index exists and is configured before rebuilding
    init_index(status_cb, warn_cb)
