    ...
    for entity in entities:
        ...
        if hasattr(entity, "component"):
            ...
            if change.old_version is None and change.new_version:
                ...
                # If the version numbers are different, this block was modified.
                # If not, it was included as a side effect of some other change, like renaming a parent container.
...
def check_container_content_changes(
    ...
    if old_version_id:
        old_version = content_api.get_container_version(old_version_id)
        old_entity_list_id = old_version.entity_list_id
    else:
    ...
    if title_changed:
        ...
        old_child_ids = old_version.entity_list.entitylistrow_set.values_list("entity_id", flat=True)
    ...
    elif old_entity_list_id == new_entity_list_id:
